...
fn main() -> Result<()> {
    ...
    let multiproof = EIP1186MultiProof::from_separate(proofs)?;

    // Modify a slot.
    let account = ethers::types::H160::from_str("0xTODO");
    let storage_key = ethers::types::H256::from_str("0xTODO");
    let storage_value = ethers::types::U256::from_str("0xTODO");
